...
$myTestNetwork  = Get-CaasNetworks -Name "My Network"
#Get all machines in that network
$myTestServers = Get-CaasDeployedServer -Network $myTestNetwork
...
foreach ($server in $myTestServers){
    ...
    $sourceImage = Get-CaasOsImages -Network $myTestNetwork | Where Id -eq $server.sourceImageId
    if ($sourceImage -eq $Null){
        $sourceImage = Get-CaasCustomerImages -Network $myTestNetwork | Where Id -eq $server.sourceImageId
    }
    ...
    if ($sourceImage -ne $Null){
        ...
        New-CaasServer -ServerDetails $newServerDetails 
