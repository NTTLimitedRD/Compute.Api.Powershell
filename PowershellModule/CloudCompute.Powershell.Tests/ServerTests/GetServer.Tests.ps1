...
Describe "Get-CaaSServer" {
    It "List Server Api Should have no filter" {
		...
		$testConnection | Verify "GET" "/caas/2.4/$($testConnection.CaaSClientId)/server/server" 1
	}
}
...
Describe "Get-CaaSServer By Network Domain Id" {
    It "List Server Api Should have Network domain id filter" {
		...
		$testConnection | Verify "GET" "/caas/2.4/$($testConnection.CaaSClientId)/server/server?networkDomainId=a4f484de-b9ed-43e4-b565-afbf69417615" 1
	}
}
...
Describe "Get-CaaSServer By Vlan Id" {
    It "List Server Api Should have Network domain id filter" {
		...
		$testConnection | Verify "GET" "/caas/2.4/$($testConnection.CaaSClientId)/server/server?vlanId=a4f484de-b9ed-43e4-b565-afbf69417615" 1
	}
}
...
Describe "Get-CaaSServer Throws Error" {
    It "List Server Api Should throw" {
		...
		$testConnection | Setup "GET" "/caas/2.4/$($testConnection.CaaSClientId)/server/server" $errorResponse 400
