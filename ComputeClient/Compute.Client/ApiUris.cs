...
// build the query string
		...
		internal static Uri CloneServerToCustomerImage(Guid orgId, string serverId, string imageName, string imageDesc)
		{
			...
			if (string.IsNullOrEmpty(imageDesc))
				...
				uri = new Uri(string.Format("{0}/server/{1}?clone={2}&desc={3}", orgId, serverId, imageName, imageDesc), 
					UriKind.Relative);
			return uri;
		}
...
// build the query string
		...
		{
			return new Uri(string.Format("{0}/network/{1}/serverFarm/{2}?delete", orgId, networkId, serverFarmId), 
				UriKind.Relative);
		}
		...
		{
			return new Uri(string.Format("{0}/network/{1}/serverFarm/{2}/addRealServer", orgId, networkId, serverFarmId), 
				UriKind.Relative);
		}
		...
		{
			return new Uri(string.Format("{0}/network/{1}/serverFarm/{2}/removeRealServer", orgId, networkId, serverFarmId), 
				UriKind.Relative);
		}
		...
		{
			return new Uri(string.Format("{0}/network/{1}/serverFarm/{2}/addProbe", orgId, networkId, serverFarmId), 
				UriKind.Relative);
		}
		...
		{
			return new Uri(string.Format("{0}/network/{1}/serverFarm/{2}/removeProbe", orgId, networkId, serverFarmId), 
				UriKind.Relative);
		}
		...
		{
			return new Uri(string.Format("{0}/network/{1}/persistenceProfile/{2}?delete", orgId, networkId, persProfileId), 
				UriKind.Relative);
		}
